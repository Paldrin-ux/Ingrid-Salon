    ...
    public function index()
{
    ...
    return view('frontend.index', compact('categories', 'employees', 'authUser'));
}
            ...
            ->map(function ($service) use ($setting) {
                ...
                try {
                    if (
                        ...
                        class_exists(\Illuminate\Support\Number::class) &&
                        method_exists(\Illuminate\Support\Number::class, 'currency')
                    ) {
                        $service->price = \Illuminate\Support\Number::currency($service->price, $setting->currency);
                        if (isset($service->sale_price)) {
                            $service->sale_price = \Illuminate\Support\Number::currency($service->sale_price, $setting->currency);
                        }
                    } else {
                        // 🟡 Fallback formatting when intl is missing
                        $currency = $setting->currency ?? '';
                        $service->price = trim($currency . ' ' . number_format($service->price ?? 0, 2));
                        if (isset($service->sale_price)) {
                            $service->sale_price = trim($currency . ' ' . number_format($service->sale_price ?? 0, 2));
                        }
                    }
                } catch (\Throwable $e) {
                    $service->price = number_format($service->price ?? 0, 2);
                    if (isset($service->sale_price)) {
                        $service->sale_price = number_format($service->sale_price, 2);
                    }
                }
    ...
    public function getEmployeeAvailability(Employee $employee, $date = null)
    {
        ...
        try {
            ...
            // Using spatie opening hours package to process data and exceptions
