...
class ProfileController extends Controller
{
    public function index()
    {
        ...
        // Initialize employeeDays as empty array
        $employeeDays = [];

        // Load employee data if user has employee record
        if ($user->employee) {
    ...
    {
        // Prevent admin from updating employee availability
        if (auth()->user()->hasRole('admin')) {
            return redirect()->back()->withErrors(['error' => 'Administrators cannot update employee availability settings.']);
        }
