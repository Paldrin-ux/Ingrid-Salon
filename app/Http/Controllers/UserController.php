...
class UserController extends Controller
{
    ...
    {
        // Get the role type from the request (either 'employee', 'customer')
    ...
    public function update(Request $request, User $user)
{
    ...
    // ✅ Removed the old "admin role cannot be removed" rule to allow demotion
    ...
    if ($request->roles) {
        ...
        // Keep admin only for user ID 1
        if ($user->id === 1 && !in_array('admin', $roles)) {
            $roles[] = 'admin';
        }

        $user->syncRoles($roles);

        // 🔥 Refresh permission cache and session
...
public function force_delete($id)
{
    ...
    // Check if user has an employee and if that employee has appointments
    if ($user->employee && $user->employee->appointments && $user->employee->appointments->count() > 0) {
        return back()->withErrors('User cannot be deleted permanently, already engaged in existing bookings!');
    }

    // Check if the user itself has appointments
    ...
    if ($user->employee) {
        ...
        if ($user->employee->holidays && $user->employee->holidays->count() > 0) {
        ...
        if ($user->employee->services() && $user->employee->services()->exists()) {
